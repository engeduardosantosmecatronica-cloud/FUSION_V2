...
import { fetchMt5Candles, fetchMt5LiveState, setMt5Stream } from '@/lib/mt5Api';
import { getChartOverlays, getLiveSignals } from '@/services/api';
...
export default function Trading() {
  ...
  // â”€â”€ WebSocket (bridge local) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const handleWsStatusChange = useCallback((status) => {
    const msgs = { connected: ['success', 'WebSocket bridge conectado'], disconnected: ['warn', 'Bridge desconectado â€” tentando reconectar...'], error: ['error', 'Erro de conexÃ£o WebSocket'], connecting: ['info', 'Conectando ao bridge local...'] };
  ...
  // â”€â”€ REST: histÃ³rico de candles â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const { data: dbCandles = [], isFetching } = useQuery({
    queryKey: ['candles', symbol, timeframe],
    queryFn: async () => {
      const t0 = Date.now();
      try {
        const result = await fetchMt5Candles(symbol, timeframe, 200);
        log('data', `HistÃ³rico MT5 carregado: ${result.length} candles`, Date.now() - t0);
        return result;
      } catch (error) {
        log('warn', `Falha ao ler candles MT5: ${error.message}`);
        return [];
      }
    },
    staleTime: 0,
    refetchInterval: MT5_POLL_MS,
  ...
  const { data: liveState } = useQuery({
    queryKey: ['mt5-live', symbol, timeframe],
    queryFn: async () => fetchMt5LiveState(symbol, timeframe),
    staleTime: 0,
    refetchInterval: MT5_POLL_MS,
    refetchIntervalInBackground: true,
    refetchOnMount: 'always',
  ...
  const { data: chartOverlays } = useQuery({
    queryKey: ['fusion-chart-overlays', symbol, timeframe],
    queryFn: async () => getChartOverlays(symbol, timeframe),
    staleTime: 0,
  ...
  const { data: fusionLiveSignals = [] } = useQuery({
    queryKey: ['fusion-live-signals'],
    queryFn: async () => getLiveSignals({ limit: 120 }),
    staleTime: 0,
  ...
  const { data: trades = [] } = useQuery({
    queryKey: ['trades'],
    queryFn: () => fusionLocalClient.entities.Trade.list('-created_date', 50),
  });

  const { data: connections = [] } = useQuery({
    queryKey: ['connections'],
    queryFn: () => fusionLocalClient.entities.MT5Connection.list('-created_date', 1),
  });
  ...
  useEffect(() => {
    if (dbCandles.length > 0) {
      load(dbCandles);
      log('info', `Buffer atualizado: ${dbCandles.length} candles em memÃ³ria`);
    }
  }, [dbCandles, load]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await setMt5Stream(symbol, timeframe, 500);
        if (alive) {
          log('success', `MT5 stream ativo: ${result?.stream?.symbol || symbol} ${result?.stream?.timeframe || timeframe}`);
          setFeedMode('live');
        }
      } catch (error) {
        if (alive) {
          log('warn', `Falha ao ativar stream MT5: ${error.message}`);
        }
      }
    })();
    return () => { alive = false; };
  }, [symbol, timeframe, log]);

  useEffect(() => {
    if (!liveState) return;
    if (liveState.tick && liveState.tick.symbol === symbol) {
      setFeedMode('live');
      setCurrentTick(liveState.tick);
      setLastPrices(prev => ({
        ...prev,
        [liveState.tick.symbol]: {
          price: liveState.tick.bid,
          change: ((liveState.tick.bid - (prev[liveState.tick.symbol]?.price || liveState.tick.bid)) / (prev[liveState.tick.symbol]?.price || liveState.tick.bid)) * 100
        }
      }));
    }
    if (liveState.current_candle && liveState.current_candle.symbol === symbol && liveState.current_candle.timeframe === timeframe) {
      updateLast(liveState.current_candle);
    }
  }, [liveState, symbol, timeframe, updateLast]);

  // Real-time subscriptions from DB
  useEffect(() => {
    const unsub = fusionLocalClient.entities.Candle.subscribe((event) => {
      if (event.data?.symbol === symbol && event.data?.timeframe === timeframe) {
        queryClient.invalidateQueries({ queryKey: ['candles', symbol, timeframe] });
      }
    });
    const unsubTrades = fusionLocalClient.entities.Trade.subscribe(() => {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0d1117] overflow-hidden text-sm">
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        {/* LEFT SIDEBAR â€” Asset list */}
        <div className="w-44 shrink-0 border-r border-[#1c2333] bg-[#161b22] hidden xl:flex flex-col">
          <AssetList selected={symbol} onChange={setSymbol} lastPrices={lastPrices} timeframe={timeframe} />
        </div>
        ...
        <div className="w-60 shrink-0 border-l border-[#1c2333] bg-[#161b22] hidden lg:flex flex-col">
            ...
            {activeTab === 'signal' && (
              <div className="h-full overflow-y-auto">
                <div className="p-3 space-y-3">
                  <FusionSignalCard symbol={symbol} timeframe={timeframe} />
                  <SignalPanel signals={fusionLiveSignals.length ? fusionLiveSignals : signals} />
                </div>
              </div>
            )}
          ...
          {connection && (
            <div className="border-t border-[#1c2333] p-3 grid grid-cols-2 gap-1.5 text-[10px] font-mono">
              ...
              <div>
                <p className="text-[#6e7681]">PatrimÃ´nio</p>
