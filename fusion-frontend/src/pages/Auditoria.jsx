...
const SORT_OPTIONS = [
  ...
  { value: 'loss_avoided', label: 'Maior prejuÃ­zo evitado' },
  { value: 'filter', label: 'Filtro' },
  { value: 'symbol', label: 'SÃ­mbolo' },
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-border bg-secondary text-muted-foreground">
              {['SÃ­mbolo','TF','Dir','Filtro','PreÃ§o Bloq.','ApÃ³s 15m','ApÃ³s 1h','ApÃ³s 3h','Resultado (pts)','ClassificaÃ§Ã£o','Lucro Perdido','PrejuÃ­zo Evitado'].map(h => (
          ...
          <tbody>
            {sorted.map(r => (
              <tr key={r.id} className="border-b border-border hover:bg-accent">
                ...
                <td className={cn('px-2 py-1.5 font-medium', r.classification === 'bom bloqueio' ? 'text-green-400' : r.classification === 'pendente' ? 'text-yellow-400' : 'text-red-400')}>
                  {r.classification}
                </td>
