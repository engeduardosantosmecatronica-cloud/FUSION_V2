...
import { isLocalAuthEnabled, localAuthConfig, loginWithLocalAuth } from '@/lib/utils';
...
export default function Register() {
  ...
  useEffect(() => {
    if (isLocalAuthEnabled()) {
      setStep('local');
      setEmail(localAuthConfig.email);
    }
  }, []);
  ...
  const handleRegister = async (e) => {
    ...
    if (isLocalAuthEnabled()) {
      setError('Cadastro desativado no modo local. Use o login administrativo.');
      return;
    }
    ...
    setLoading(true);
    try {
      await fusionLocalClient.auth.register({ email, password });
      setStep('otp');
    } catch (err) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (isLocalAuthEnabled()) {
      try {
        loginWithLocalAuth(localAuthConfig.email, localAuthConfig.password);
    ...
    setLoading(true);
    try {
      const res = await fusionLocalClient.auth.verifyOtp({ email, otpCode });
      fusionLocalClient.auth.setToken(res.access_token);
      window.location.href = '/';
    } catch (err) {
  ...
  const handleResend = async () => {
    ...
    await fusionLocalClient.auth.resendOtp(email);
  };

  const handleGoogle = () => {
    if (isLocalAuthEnabled()) {
      setError('Login com Google está desativado no modo local.');
      return;
    }
    fusionLocalClient.auth.loginWithProvider('google', '/');
  };
      ...
      <Card className="w-full max-w-sm bg-card border-border">
        <CardHeader className="text-center space-y-2">
          ...
          <CardDescription className="text-xs">
            {isLocalAuthEnabled() ? 'Modo local ativo. Use o login administrativo.' : (step === 'register' ? 'Preencha os dados abaixo' : 'Verifique seu email')}
          </CardDescription>
        </CardHeader>
          ...
          {isLocalAuthEnabled() ? (
            <div className="space-y-3 text-center">
              <p className="text-xs text-muted-foreground leading-relaxed">
                O cadastro está desativado no modo local. Entre com:
              </p>
              <div className="rounded-md border border-border bg-muted p-3 text-left text-xs space-y-1">
                <p><span className="text-muted-foreground">Email:</span> {localAuthConfig.email}</p>
                <p><span className="text-muted-foreground">Senha:</span> {localAuthConfig.password}</p>
              </div>
