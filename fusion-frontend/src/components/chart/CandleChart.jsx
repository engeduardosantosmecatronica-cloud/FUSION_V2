...
function sliceSeries(values, start, count) {
  return Array.isArray(values) ? values.slice(start, start + count) : values;
}

function sliceIndicators(indicators = {}, start, count) {
  const sliced = {};
  if (indicators.ema1) sliced.ema1 = { ...indicators.ema1, values: sliceSeries(indicators.ema1.values, start, count) };
  if (indicators.ema2) sliced.ema2 = { ...indicators.ema2, values: sliceSeries(indicators.ema2.values, start, count) };
  if (indicators.ema3) sliced.ema3 = { ...indicators.ema3, values: sliceSeries(indicators.ema3.values, start, count) };
  if (indicators.rsi) sliced.rsi = { ...indicators.rsi, values: sliceSeries(indicators.rsi.values, start, count) };
  if (indicators.boll) {
    sliced.boll = {
      ...indicators.boll,
      upper: sliceSeries(indicators.boll.upper, start, count),
      middle: sliceSeries(indicators.boll.middle, start, count),
      lower: sliceSeries(indicators.boll.lower, start, count),
    };
  }
  if (indicators.macd) {
    sliced.macd = {
      ...indicators.macd,
      macd: sliceSeries(indicators.macd.macd, start, count),
      signal: sliceSeries(indicators.macd.signal, start, count),
      histogram: sliceSeries(indicators.macd.histogram, start, count),
    };
  }
  return sliced;
}
export default function CandleChart({ candles, indicators, currentTick, overlays }) {
  ...
  const scheduleDraw = useCallback(() => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      if (visible.length === 0) {
        ...
        ctx.fillText('Aguardando dados â€” clique em "Gerar Demo" para comeÃ§ar', canvas.clientWidth / 2, canvas.clientHeight / 2);
        return;
      }
      const viewStart = bufferRef.current.viewStart || 0;
      const visibleIndicators = sliceIndicators(indicators, viewStart, visible.length);
      metaRef.current = renderChart(canvas, visible, visibleIndicators, crosshair, overlays);
    });
  }, [indicators, crosshair, overlays]);
  ...
  // Redraw when crosshair moves (only affects overlay layer â€” cheap)
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full bg-[#0d1117] overflow-hidden">
      ...
      <div className="absolute bottom-9 left-2 text-[10px] text-[#3d4553] pointer-events-none select-none">
        Scroll para zoom Â· Arrastar para pan
      </div>
    </div>
  );
}
