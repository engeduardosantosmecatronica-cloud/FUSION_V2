/**
 * Renderizadores genÃ©ricos de campos para o Fusion Control Center
...
/** Input numÃ©rico com unidade opcional */
    ...
    <div className="space-y-1">
      {entries.map(([k, v]) => (
        <div key={k} className="flex items-center gap-1">
          ...
          />
          <button onClick={() => remove(k)} className="text-xs text-red-400 hover:text-red-300">Ã—</button>
        </div>
      ))}
...
function normalizeChipListValue(value) {
...
function ChipListEditorInner({ value = [], onChange }) {
  const items = normalizeChipListValue(value);
  ...
  const remove = (item) => onChange(items.filter(v => v !== item));
  return (
    <div className="space-y-1">
      <div className="flex flex-wrap gap-1">
        {items.map(v => (
          <span key={v} className="flex items-center gap-1 px-2 py-0.5 bg-secondary border border-border rounded text-xs">
            {v}
            <button onClick={() => remove(v)} className="text-muted-foreground hover:text-red-400 leading-none">x</button>
          </span>
        ))}
      </div>
...
/** Renderizador genÃ©rico baseado em tipo/nome do campo */
