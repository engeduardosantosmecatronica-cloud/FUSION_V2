...
class Estrategia13(BaseStrategy):
    name = "strategy13"
    tag = "S13"
    ...
    def _smma(series: pd.Series, period: int) -> pd.Series:
        return series.astype(float).ewm(alpha=1 / period, adjust=False).mean()

    @staticmethod
    def _fractals(frame: pd.DataFrame, idx: int, left: int = 2, right: int = 2) -> tuple[list[int], list[int]]:
        highs: list[int] = []
        lows: list[int] = []
        high = frame["high"].astype(float)
        low = frame["low"].astype(float)
    ...
    def _setup(self, context: StrategyContext) -> tuple[int, dict[str, Any]]:
        ...
        highs, lows = self._fractals(frame, idx, int(cfg.get("fractal_left", 2) or 2), int(cfg.get("fractal_right", 2) or 2))
        last_high = highs[-1] if highs else None
        last_low = lows[-1] if lows else None
        mouth_open = abs(float(lips.iloc[idx]) - float(jaw.iloc[idx])) / max(abs(float(close.iloc[idx])), 1e-12) >= float(cfg.get("min_alligator_spread_pct", 0.0005) or 0.0005)
        bullish_alligator = float(lips.iloc[idx]) > float(teeth.iloc[idx]) > float(jaw.iloc[idx])
        bearish_alligator = float(lips.iloc[idx]) < float(teeth.iloc[idx]) < float(jaw.iloc[idx])
        ao_now = float(ao.iloc[idx]) if np.isfinite(ao.iloc[idx]) else 0.0
        ac_now = float(ac.iloc[idx]) if np.isfinite(ac.iloc[idx]) else 0.0
        buy_fractal_break = last_high is not None and float(close.iloc[idx]) > float(high.iloc[last_high])
        sell_fractal_break = last_low is not None and float(close.iloc[idx]) < float(low.iloc[last_low])
        buy_ok = mouth_open and bullish_alligator and buy_fractal_break and ao_now > 0 and ac_now > 0
        sell_ok = mouth_open and bearish_alligator and sell_fractal_break and ao_now < 0 and ac_now < 0
        pred = 1 if buy_ok else 2 if sell_ok else 0
        metadata = {"setup": "bill_williams_alligator_fractals_ao_ac", "close": float(close.iloc[idx]), "jaw": float(jaw.iloc[idx]), "teeth": float(teeth.iloc[idx]), "lips": float(lips.iloc[idx]), "mouth_open": mouth_open, "ao": ao_now, "ac": ac_now, "last_high_fractal": last_high, "last_low_fractal": last_low, "signal_time": str(frame.iloc[idx].get("time", ""))}
        if pred == 0:
            metadata.update({"reason": "bill_williams_sem_confirmacao", "buy_ok": buy_ok, "sell_ok": sell_ok, "buy_fractal_break": buy_fractal_break, "sell_fractal_break": sell_fractal_break})
            return 0, metadata
        if not self._model_confirms(pred, context, metadata):
            return 0, metadata
        metadata["reason"] = "bill_williams_confirmado"
