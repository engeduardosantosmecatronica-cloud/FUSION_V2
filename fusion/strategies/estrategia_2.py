...
class Estrategia2(BaseStrategy):
    name = "strategy2"
    tag = "S2"
    ...
    def evaluate(self, context: StrategyContext, last_trade_time: dict) -> StrategyDecision:
        ...
        feature_row = self.app._strategy2_feature_candidate(context.symbol, context.timeframe, pred, context.broker_symbol)
        if not feature_row:
            self.app.logger.info(f"S2 sem feature aprovada: {context.symbol} {context.timeframe}")
