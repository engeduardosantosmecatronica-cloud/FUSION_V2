...
class Estrategia9(BaseStrategy):
    name = "strategy9"
    tag = "S9"
    ...
    def _setup(self, context: StrategyContext) -> tuple[int, dict[str, Any]]:
        ok, reason = self._allowed(context, ["EURUSD", "GBPUSD", "USDJPY", "GOLD", "XAUUSD"], ["M15", "M30", "H1"])
        if not ok:
            return 0, reason
        cfg = self._cfg()
        ...
        upper = mid + float(cfg.get("bb_std_mult", 2.0) or 2.0) * std
        lower = mid - float(cfg.get("bb_std_mult", 2.0) or 2.0) * std
        rsi = self._rsi(close, int(cfg.get("rsi_period", 14) or 14))
        cci = self._cci(frame, int(cfg.get("cci_period", 20) or 20))
        ...
        cci_now = float(cci.iloc[idx]) if np.isfinite(cci.iloc[idx]) else 0.0
        atr_not_exploding = atr_now <= atr_ma * float(cfg.get("max_atr_vs_ma", 1.25) or 1.25)
        lateral_ok = adx_now <= float(cfg.get("max_adx", 24.0) or 24.0)
        buy_ok = price <= float(lower.iloc[idx]) and rsi_now <= float(cfg.get("rsi_oversold", 30.0) or 30.0) and cci_now <= -abs(float(cfg.get("cci_extreme", 100.0) or 100.0)) and atr_not_exploding and lateral_ok
        sell_ok = price >= float(upper.iloc[idx]) and rsi_now >= float(cfg.get("rsi_overbought", 70.0) or 70.0) and cci_now >= abs(float(cfg.get("cci_extreme", 100.0) or 100.0)) and atr_not_exploding and lateral_ok
        pred = 1 if buy_ok else 2 if sell_ok else 0
        metadata = {"setup": "mean_reversion_bb_rsi_cci_atr", "close": price, "bb_upper": float(upper.iloc[idx]), "bb_lower": float(lower.iloc[idx]), "rsi14": rsi_now, "cci": cci_now, "atr": atr_now, "atr_ma": atr_ma, "adx": adx_now, "atr_not_exploding": atr_not_exploding, "lateral_ok": lateral_ok, "signal_time": str(frame.iloc[idx].get("time", ""))}
        if pred == 0:
            metadata.update({"reason": "sem_reversao_media_confirmada", "buy_ok": buy_ok, "sell_ok": sell_ok})
            return 0, metadata
        if not self._model_confirms(pred, context, metadata):
            return 0, metadata
        metadata["reason"] = "reversao_media_confirmada"
