...
export default function MT5DataImporter({ symbol, timeframe, onImported }) {
  ...
  const handleJsonImport = async () => {
    ...
    await base44.entities.Candle.bulkCreate(formatted);
    setIsLoading(false);
    setOpen(false);
    onImported?.();
  };
  ...
  const generateSampleData = async () => {
    ...
    for (let i = 0; i < candles.length; i += 50) {
      await base44.entities.Candle.bulkCreate(candles.slice(i, i + 50));
    }
        ...
        <Tabs defaultValue="sample">
          ...
          <TabsContent value="sample" className="mt-4 space-y-3">
            <p className="text-xs text-muted-foreground">
              Gera 200 candles de exemplo para {symbol} ({timeframe}) para testar o gráfico.
            </p>
