      ...
      <div className="bg-card border border-border rounded p-4 w-80" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-2">
          ...
          {action !== 'close' && action !== 'trailing' && (
            <input type="number" step="0.00001" placeholder={action === 'partial' ? 'Lots' : 'Preço'}
          ...
          {msg ? <div className="text-xs text-green-400 font-medium">{msg}</div> : (
            <button onClick={submit} className="w-full text-xs py-2 bg-primary text-primary-foreground rounded">Executar (Simulado)</button>
          )}
        </div>
      </div>
    </div>
...
export default function OrdensMT5() {
  ...
  const load = () => getOpenOrders().then(setOrders);
  useEffect(() => { load(); }, []);
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-border bg-secondary text-muted-foreground">
              {['Ticket','Símbolo','Dir','Lots','Entrada','Atual','SL','TP','P/L','Magic','Estratégia','TF','Abertura','Trail','Ação'].map(h => (
